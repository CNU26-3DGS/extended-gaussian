...
namespace {
const sibr::AssetDescriptor *
manifestDescriptor(const sibr::ManifestStore *manifest,
                   const sibr::AssetId &id) {
  if (!manifest) {
    return nullptr;
  }
...
int assetPriority(const sibr::ManifestStore *manifest,
                  const sibr::AssetId &id) {
...
namespace sibr {
SwapManager::SwapManager(ResourceManager &registry,
                         GPUResourceManager &gpuManager)
    : registry_(registry), gpuManager_(gpuManager) {}

SwapManager::~SwapManager() { worker_.stop(); }
...
void SwapManager::tick(const ViewerContext &context) {
  ...
  if (!hasManifest()) {
    stats_.pending_disk_loads =
        worker_.pendingCount() + worker_.inFlightCount();
...
bool SwapManager::isCpuDesired(const AssetId &id,
                               const PolicyResult &result) const {
  return result.required_gpu.count(id) > 0 || result.warm_cpu.count(id) > 0 ||
         result.protected_assets.count(id) > 0;
}

bool SwapManager::isGpuDesired(const AssetId &id,
                               const PolicyResult &result) const {
...
void SwapManager::updateDesiredStates(const PolicyResult &result) {
  for (const auto &assetPair : manifest_->assets()) {
    registry_.setDesiredCpu(assetPair.first,
                            isCpuDesired(assetPair.first, result));
  }
}

void SwapManager::updateEvictableTimers(const PolicyResult &result,
                                        const ViewerContext &context) {
  for (const auto &assetPair : manifest_->assets()) {
    const auto &id = assetPair.first;
    if (isGpuDesired(id, result)) {
...
void SwapManager::scheduleCpuLoads(const PolicyResult &result) {
  ...
  for (const auto &assetPair : manifest_->assets()) {
    ...
    if (registry_.isCpuResident(id) ||
        registry_.cpuState(id) == CpuState::Loading) {
      continue;
    }

    const size_t estimatedBytes = assetPair.second.estimated_cpu_bytes;
    if (targetBytes > 0 && estimatedBytes > 0 &&
        residentBytes + estimatedBytes > targetBytes) {
      const size_t bytesNeeded = residentBytes + estimatedBytes - targetBytes;
      residentBytes -=
          std::min(residentBytes, evictCpuForBudget(result, bytesNeeded));
    }
    if (targetBytes > 0 && estimatedBytes > 0 &&
        residentBytes + estimatedBytes > targetBytes) {
      continue;
...
void SwapManager::scheduleGpuUploads(const PolicyResult &result) {
  std::vector<AssetId> uploadOrder(result.required_gpu.begin(),
                                   result.required_gpu.end());
  std::sort(uploadOrder.begin(), uploadOrder.end(),
  ...
  const size_t uploadBudget =
      manifest_ ? manifest_->settings().max_upload_bytes_per_frame : 0;
  size_t residentBytes = gpuManager_.totalBytes();
  const size_t targetBytes =
      manifest_ ? manifest_->settings().target_vram_bytes : 0;
  ...
  for (const auto &id : uploadOrder) {
    ...
    if (sizeKnown && uploadBudget > 0 &&
        uploadedBytes + estimatedBytes > uploadBudget) {
      ++stats_.pending_gpu_uploads;
      continue;
    }
    if (sizeKnown && targetBytes > 0 &&
        residentBytes + estimatedBytes > targetBytes) {
      const size_t bytesNeeded = residentBytes + estimatedBytes - targetBytes;
      residentBytes -=
          std::min(residentBytes, evictGpuForBudget(result, bytesNeeded));
    }
    if (sizeKnown && targetBytes > 0 &&
        residentBytes + estimatedBytes > targetBytes) {
      ++stats_.pending_gpu_uploads;
...
size_t SwapManager::evictGpuForBudget(const PolicyResult &result,
                                      size_t bytesNeeded) {
  if (bytesNeeded == 0) {
    return 0;
  }
  ...
  for (const auto &status : gpuManager_.snapshot()) {
    ...
    if (isGpuDesired(status.id, result) ||
        result.protected_assets.count(status.id) > 0) {
      continue;
    }
  ...
  std::sort(candidates.begin(), candidates.end(),
            [this](const GPUResourceManager::GpuAssetStatus &lhs,
                   const GPUResourceManager::GpuAssetStatus &rhs) {
  ...
  const size_t budgetSetting =
      manifest_ ? manifest_->settings().max_gpu_evictions_per_frame : 0;
  size_t remainingBudget =
      (budgetSetting == 0) ? std::numeric_limits<size_t>::max() : budgetSetting;
...
void SwapManager::scheduleGpuEvictions(const PolicyResult &result,
                                       const ViewerContext &context) {
  std::vector<GPUResourceManager::GpuAssetStatus> candidates;
...
void SwapManager::scheduleCpuEvictions(const PolicyResult &result,
                                       const ViewerContext &context) {
  ...
  const size_t cpuBudgetSetting =
      manifest_ ? manifest_->settings().max_cpu_evictions_per_frame : 0;
  size_t remainingBudget = (cpuBudgetSetting == 0)
  ...
  for (const auto &asset : assets) {
    ...
    if (!asset.cpu_resident || isCpuDesired(asset.id, result) ||
        asset.pinned_cpu) {
      continue;
    }
...
size_t SwapManager::evictCpuForBudget(const PolicyResult &result,
                                      size_t bytesNeeded) {
  if (bytesNeeded == 0) {
    return 0;
  }
  ...
  std::sort(assets.begin(), assets.end(),
            [](const AssetStatus &lhs, const AssetStatus &rhs) {
              if (lhs.priority != rhs.priority) {
                return lhs.priority < rhs.priority;
              }
              return lhs.actual_cpu_bytes > rhs.actual_cpu_bytes;
