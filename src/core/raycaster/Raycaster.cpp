...
namespace sibr
{
	namespace
	{
		...
		void rtcIntersect8Compat(const int* valid, RTCScene scene, RTCRayHit8* rayhit)
		{
#if RTC_VERSION_MAJOR >= 4
			rtcIntersect8(valid, scene, rayhit, nullptr);
#else
			RTCIntersectContext context;
			rtcInitIntersectContext(&context);
		...
		void rtcOccluded1Compat(RTCScene scene, RTCRay* ray)
		{
#if RTC_VERSION_MAJOR >= 4
		...
		void rtcOccluded8Compat(const int* valid, RTCScene scene, RTCRay8* ray)
		{
#if RTC_VERSION_MAJOR >= 4
			rtcOccluded8(valid, scene, ray, nullptr);
#else
			RTCIntersectContext context;
			rtcInitIntersectContext(&context);
	...
	bool	Raycaster::hitSomething(const Ray& inray, float minDist)
	{
		...
		{
			rtcOccluded1Compat(*_scene.get(), &ray);
		}
	...
	std::array<bool, 8>	Raycaster::hitSomething8(const std::array<Ray, 8> & inray, float minDist)
	{
		...
		{
			rtcOccluded8Compat(valid8, *_scene.get(), &ray);
		}
	...
	RayHit	Raycaster::intersect(const Ray& inray, float minDist)
	{
		...
		{
			rtcIntersect1Compat(*_scene.get(), &rh);
	...
	std::array<RayHit, 8>	Raycaster::intersect8(const std::array<Ray, 8> & inray, const std::vector<int> & valid8, float minDist)
	{
		...
		{
			rtcIntersect8Compat(valid8.data(), *_scene.get(), &rh);
		}
