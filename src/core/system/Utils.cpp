...
#define DIV 1024
	...
	{
		const auto resolveInstallDirectory = [](const std::string& hint) {
			boost::filesystem::path current = boost::filesystem::absolute(hint);

			while (!current.empty()) {
				const std::string candidate = current.string();
				if (directoryExists(candidate + "/bin")
					|| directoryExists(candidate + "/resources")
					|| directoryExists(candidate + "/shaders")) {
					return candidate;
				}

				if (directoryExists(candidate + "/install/bin")
					|| directoryExists(candidate + "/install/resources")
					|| directoryExists(candidate + "/install/shaders")) {
					return (current / "install").string();
				}

				const boost::filesystem::path parent = current.parent_path();
				if (parent == current) {
					break;
				}
				current = parent;
			...
			return boost::filesystem::absolute(hint).string();
		};

		const auto& commandLineArgs = getCommandLineArgs();
		if (commandLineArgs.contains("appPath")) {
			const std::string customAppPath = commandLineArgs.get<std::string>("appPath", "");
			if (!customAppPath.empty() && customAppPath != "." && customAppPath != "./") {
				return resolveInstallDirectory(customAppPath);
			}
		}
		char exePath[4095];
		...
		std::string installDirectory = resolveInstallDirectory(parentDirectory(parentDirectory(exePath)));
#else
		unsigned int len=0;
		...
		std::string installDirectory(resolveInstallDirectory(parentDirectory(path)));
#endif
